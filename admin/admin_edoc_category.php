<table height="28" cellSpacing="0" cellPadding="0" width="100%" border="0">
      <tr align=center>
        ...
        Danh m&#7909;c File : <b> <a href="./?act=edoc_category_m"><font color="#FFFFFF">Nh&#7853;p 
...
<?
	switch ($_GET['action'])
	{
		case 'del' :
			$id = $_GET['id'];
			$result = mysql_query("select doc_images_categoriesid from doc_images where doc_images_categoriesid='".$id."'",$con);
			if (mysql_num_rows($result)<=0) {
				$sql = "delete from doc_categories where doc_categories_id='".$id."'";
...
<?
	if (isset($_POST['ButDel'])) {
		...
		{
			@$result = mysql_query("delete from doc_categories where doc_categories_id='".$id."'",$con);
			if ($result) $cnt++;
		}
...
<input type="hidden" name="act" value="edoc_category">
...
{	global $con;
	...
	$pageindex=taotrang("select count(*) from doc_categories","./?act=edoc_category"."&page=",$MAXPAGE,$page);
?>
...
<table border="1" cellpadding="2" style="border-collapse: collapse" bordercolor="#C9C9C9" width="100%" id="AutoNumber1">
  ...
  <?
            	$sql="select *,DATE_FORMAT(date_added,'%d/%m/%Y %h:%m') as dateformat from doc_categories order by doc_categories_id limit ".($p*$MAXPAGE).",".$MAXPAGE;
            	$result=mysql_query($sql,$con);
            	$i=0;
            	while(($row=mysql_fetch_array($result)))
				{
					...
					$parentname="";
					if ($row['doc_categories_parentid']>0)
					{
						$sqlcat="select * from doc_categories where doc_categories_id='".$row['doc_categories_parentid']."' limit 1";
						$rowsub=mysql_fetch_array(mysql_query($sql,$con));
						$parentname=$rowsub['doc_categories_name'];
					}
					else
  ...
  <tr>
    ...
    <input type="checkbox" name="chk[]" value="<? echo $row['doc_categories_id']; ?>"></td>
    <td width="20" bgcolor="<? echo $color; ?>" class="smallfont">
    <a href="./?act=edoc_category_m&page=<? echo $_REQUEST['page']; ?>&id=<? echo $row['doc_categories_id']; ?>">S&#7917;a</a></td>
    <td width="20" bgcolor="<? echo $color; ?>" class="smallfont">
    <a onclick="return confirm('B&#7841;n có ch&#7855;c ch&#7855;n mu&#7889;n xoá ?');" href="./?act=edoc_category&page=<? echo $_REQUEST['page']; ?>&action=del&id=<? echo $row['doc_categories_id']; ?>">Xoá</a></td>
    <td width="20" bgcolor="<? echo $color; ?>" align="left" width="55" align="left" class="smallfont"><? echo $row['doc_categories_id']; ?>&nbsp;</td>
    <td bgcolor="<? echo $color; ?>" class="smallfont"><? echo $row['doc_categories_name']; ?>&nbsp;</td>
    <td bgcolor="<? echo $color; ?>" class="smallfont"><? echo $parentname; ?>&nbsp;</td>
    <td width="80" bgcolor="<? echo $color; ?>" class="smallfont"><? echo $row['doc_categories_status']; ?>&nbsp;</td>
