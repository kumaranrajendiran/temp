<table height="28" cellSpacing="0" cellPadding="0" width="100%" border="0">
      <tr align=center>
        <td class="title" width="100%">Website : <a href="./?act=link_website_m&page=<? echo $_REQUEST['page']; ?>"><font class="V10pt" color="#ffffff">Nh&#7853;p M&#7899;i</font></a>&nbsp;&nbsp;
	</td>
      </tr>
    </table>
<?
	switch ($_GET['action'])
	{
		case 'del' :
			...
			if ($pro)
			{
				$sql = "delete from link_websites where link_websites_id='".$id."'";
...
<?
	if (isset($_POST['ButDel'])) {
		...
		foreach ($_POST['chk'] as $id)
		{
			...
			if ($pro)
			{
				@$result = mysql_query("delete from link_websites where link_websites_id='".$id."'",$con);
				if ($result) {
					$cnt++;
					if (file_exists($pro['link_websites_img'])) unlink($pro['link_websites_img']);
					if (file_exists($pro['link_websites_imglarge'])) unlink($pro['link_websites_imglarge']);
...
{	global $con;
	...
	$pageindex=taotrang("select count(*) from link_websites","./?act=link_website"."&page=",$MAXPAGE,$page);
?>
...
<table border="1" cellpadding="2" style="border-collapse: collapse" bordercolor="#C9C9C9" width="100%" id="AutoNumber1">
  <tr>
    ...
    <td align="center" nowrap class="title"><b>Ngày nh&#7853;p </b></td>
	<td align="center" nowrap class="title"><b>Ng&agrave;y s&#7917;a </b></td>
	<td align="center" nowrap class="title"><strong>Ng&ocirc;n ng&#7919; </strong></td>
  </tr>
  
  <?php
           	$sql="select * from link_websites order by link_websites_modified desc limit ".($p*$MAXPAGE).",".$MAXPAGE;
        	$result=mysql_query($sql,$con);
        	$i=0;
  ...
  <tr>
    ...
    <td align="center" bgcolor="<? echo $color; ?>" class="smallfont">
    <a href="./?act=link_website_m&id=<? echo $row['link_websites_id']; ?>&page=<? echo $_REQUEST['page'];?>">S&#7917;a</a></td>
    <td align="center" bgcolor="<? echo $color; ?>" class="smallfont">
    <a onclick="return confirm('B&#7841;n có ch&#7855;c ch&#7855;n mu&#7889;n xoá ?');" href="./?act=link_website&page=<? echo $_REQUEST['page']; ?>&action=del&id=<? echo $row['link_websites_id']; ?>">Xóa</a></td>
    ...
    <td bgcolor="<? echo $color; ?>" class="smallfont"><? echo $row['link_websites_dateadded']; ?>&nbsp;</td>
	<td bgcolor="<? echo $color; ?>" class="smallfont"><? echo $row['link_websites_modified']; ?>&nbsp;</td>
	<td bgcolor="<? echo $color; ?>" class="smallfont"><? echo $row['link_websites_language']; ?>&nbsp;</td>
  </tr>
  <?
              	}
  ?>
