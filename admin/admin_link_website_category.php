...
{	global $con;
	...
	$pageindex=taotrang("select count(*) from link_websites_categories where link_websites_categories_parentid>2","./?act=link_website_category"."&page=",$MAXPAGE,$page);
?>
...
<table border="1" cellpadding="2" style="border-collapse: collapse" bordercolor="#C9C9C9" width="100%" id="AutoNumber1">
  ...
  <?
            	$sql="select *,DATE_FORMAT(date_added,'%d/%m/%Y %h:%m') as dateformat from link_websites_categories where link_websites_categories_parentid >0 order by date_added desc limit ".($p*$MAXPAGE).",".$MAXPAGE;
            	$result=mysql_query($sql,$con);
            	$i=0;
  ...
  <tr>
    ...
    <td width="20" bgcolor="<? echo $color; ?>" class="smallfont">
    <a onclick="return confirm('B&#7841;n có ch&#7855;c ch&#7855;n mu&#7889;n xoá ?');" href="./?act=link_website_category&page=<? echo $_REQUEST['page']; ?>&action=del&id=<? echo $row['link_websites_categories_id']; ?>">Xóa</a></td>
