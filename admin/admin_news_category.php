...
<table height="28" cellSpacing="0" cellPadding="0" width="100%" border="0">
      <tr align=center>
        ...
        Danh m&#7909;c: <b> <a href="./?act=news_category_m"><font color="#FFFFFF">Nh&#7853;p 
...
<?
	switch ($_GET['action'])
	{
		case 'del' :
			$id = $_GET['id'];
			$result = mysql_query("select categories_id from news where categories_id='".$id."'",$con);
			if (mysql_num_rows($result)<=0) {
				$sql = "delete from newscategories where categories_id='".$id."'";
...
<?
	if (isset($_POST['ButDel'])) {
		...
		{
			@$result = mysql_query("delete from newscategories where categories_id='".$id."'",$con);
			if ($result) $cnt++;
		}
...
{	global $con;
	...
	$pageindex=taotrang("select count(*) from newscategories","./?act=news_category"."&page=",$MAXPAGE,$page);
?>
...
<table border="1" cellpadding="2" style="border-collapse: collapse" bordercolor="#C9C9C9" width="100%" id="AutoNumber1">
  ...
  <?
            	$sql="select * from newscategories where parent_id!=0 and parent_id!=1 order by sort_order limit ".($p*$MAXPAGE).",".$MAXPAGE;
            	$result=mysql_query($sql,$con);
            	$i=0;
  ...
  <tr>
    ...
    <a href="./?act=news_category_m&page=<? echo $_REQUEST['page']; ?>&id=<? echo $row['categories_id']; ?>">S&#7917;a</a></td>
    <td width="20" bgcolor="<? echo $color; ?>" class="smallfont">
    <a onclick="return confirm('B&#7841;n có ch&#7855;c ch&#7855;n mu&#7889;n xoá ?');" href="./?act=news_category&page=<? echo $_REQUEST['page']; ?>&action=del&id=<? echo $row['categories_id']; ?>">Xoá</a></td>
    ...
    <td width="80" bgcolor="<? echo $color; ?>" class="smallfont"><? echo $row['status']; ?>&nbsp;</td>
    <td width="100" bgcolor="<? echo $color; ?>" class="smallfont"><? echo date('d/m/Y',strtotime($row['date_added'])); ?>&nbsp;</td>
  </tr>
  <?
              	}
  ?>
