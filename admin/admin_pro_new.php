...
<?
	switch ($_GET['action'])
	{
		case 'del' :
			$id = $_GET['id'];
			$sql = "delete from pro_new where pro_id='".$id."'";
...
<?
	if (isset($_POST['ButDel'])) {
		...
		{
			@$result = mysql_query("delete from pro_new where pro_id='".$id."'",$con);
			if ($result) $cnt++;
		}
...
{	global $con;
	...
	$pageindex=taotrang("select count(*) from pro_new","./?act=pro_new&cat=".$_REQUEST['cat']."&page=",$MAXPAGE,$page);
?>
...
<table border="1" cellpadding="2" style="border-collapse: collapse" bordercolor="#C9C9C9" width="100%" id="AutoNumber1">
  ...
  <?
            	$sql="select *,DATE_FORMAT(pro_dateadded,'%d/%m/%Y %h:%m') as dateformat from pro_new order by pro_id limit ".($p*$MAXPAGE).",".$MAXPAGE;
            	$result=mysql_query($sql,$con);
            	$i=0;
  ...
  <tr>
    ...
    <a href="./?act=pro_new_m&id=<? echo $row['pro_id']; ?>&page=<? echo $page?>">S&#7917;a</a></td>
    </td>
    <td width="20" bgcolor="<? echo $color; ?>" class="smallfont">
    <a onclick="return confirm('B&#7841;n có ch&#7855;c ch&#7855;n mu&#7889;n xoá ?');" href="./?act=pro_new&action=del&page=<? echo $_REQUEST['page']; ?>&id=<? echo $row['pro_id']; ?>">Xoá</a></td>
...
<input type="hidden" name="act" value="pro_new">
</form>
