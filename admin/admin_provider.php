<table height="28" cellSpacing="0" cellPadding="0" width="100%" border="0">
      <tr align=center>
        ...
        Nhà cung c&#7845;p : <b> <a href="./?act=provider_m"><font color="#FFFFFF">Nh&#7853;p 
...
<?
	switch ($_GET['action'])
	{
		case 'del' :
			$id = $_GET['id'];
			$result = mysql_query("select products_id from products where providers_id='".$id."'",$con);
			if (mysql_num_rows($result)<=0) {
				$sql = "delete from providers where providers_id='".$id."'";
...
<?
	if (isset($_POST['ButDel'])) {
		...
		{
			@$result = mysql_query("delete from providers where providers_id='".$id."'",$con);
			if ($result) $cnt++;
		}
...
{	global $con;
	...
	$pageindex=taotrang("select count(*) from providers","./?act=provider"."&page=",$MAXPAGE,$page);
?>
...
<table border="1" cellpadding="2" style="border-collapse: collapse" bordercolor="#C9C9C9" width="100%" id="AutoNumber1">
  <tr>
    ...
    <td align="center" nowrap class="title"><b>Tên nhà cung c&#7845;p</b></td>
  ...
  <?
            	$sql="select *,DATE_FORMAT(providers_dateadded,'%d/%m/%Y %h:%m') as dateformat from providers order by providers_id limit ".($p*$MAXPAGE).",".$MAXPAGE;
            	$result=mysql_query($sql,$con);
            	$i=0;
  ...
  <tr>
    ...
    <input type="checkbox" name="chk[]" value="<? echo $row['providers_id']; ?>"></td>
    <td width="20" bgcolor="<? echo $color; ?>" class="smallfont">
    <a href="./?act=provider_m&page=<? echo $_REQUEST['page']; ?>&id=<? echo $row['providers_id']; ?>">Edit</a></td>
    <td width="20" bgcolor="<? echo $color; ?>" class="smallfont">
    <a onclick="return confirm('B&#7841;n có ch&#7855;c ch&#7855;n mu&#7889;n xoá ?');" href="./?act=provider&action=del&page=<? echo $_REQUEST['page']; ?>&id=<? echo $row['providers_id']; ?>">Delete</a></td>
    <td width="20" bgcolor="<? echo $color; ?>" align="left" width="55" align="left" class="smallfont"><? echo $row['providers_id']; ?>&nbsp;</td>
    <td bgcolor="<? echo $color; ?>" class="smallfont"><? echo $row['providers_name']; ?>&nbsp;</td>
    <td bgcolor="<? echo $color; ?>" class="smallfont"><? echo $row['providers_address']; ?>&nbsp;</td>
    <td width="80" bgcolor="<? echo $color; ?>" class="smallfont"><? echo $row['providers_phone']; ?>&nbsp;</td>
    <td width="80" bgcolor="<? echo $color; ?>" class="smallfont"><? echo $row['providers_email']; ?>&nbsp;</td>
    <td width="80" bgcolor="<? echo $color; ?>" class="smallfont"><? echo $row['providers_status']; ?>&nbsp;</td>
